...
missing_val_fix <-function(data, rep_set_1, rep_set_others){
  ...
  data <- data %>% replace_na(rep_1)
  for (i in rep_set_others) {
    
  }
  data <- data %>% 
              mutate_at(rep_2, ~replace_na(., rep_2_val)) %>% 
              mutate_at(rep_3, ~replace_na(., rep_3_val)) %>% 
              mutate_at(rep_4, ~replace_na(., rep_4_val)) %>% 
              mutate_at(rep_5, ~replace_na(., rep_5_val))
...
rep_set_others <- list(
  set_2 <- list("Unknown"=c('MSZoning', 'Utilities', 'Exterior1st', 
        'Exterior2nd', 'MasVnrType', 'KitchenQual',
        'Functional', 'SaleType')),
  
  set_3 <-list("No Basement"=c('BsmtQual', 'BsmtCond', 'BsmtExposure', 
        'BsmtFinType1', 'BsmtFinType2')),
  
  set_4 <-list("No Garage"=c( 'GarageType', 'GarageFinish', 'GarageQual',
        'GarageCond')),
  
  set_5 <-list(0 = c(  'MasVnrArea', 'BsmtFinSF1', 'BsmtFinSF2', 
