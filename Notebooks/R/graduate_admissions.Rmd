...
```{r}
#' basic_processing <- function(data, col.drop) {
#' #' Basic data processing: drop/rename columns, remove duplicate(s)
#' #'
#' # @param data: initial dataframe
#' # @param col_drop: vector with column names to drop
#' #
#' # @return data: modified dataframe
#'   data <- data %>% select(-col.drop)
#'           #rename("Chance.of.Admit"= "target") %>%
#'           
#'   
#'   return(data)
#' }
#' col.drop <- c(Serial.No.)
#' basic_processing(df, col.drop = col.drop)
```
